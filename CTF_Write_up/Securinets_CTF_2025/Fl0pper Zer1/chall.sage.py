...
_sage_const_1 = Integer(1); _sage_const_16 = Integer(16); _sage_const_3 = Integer(3); _sage_const_5 = Integer(5); _sage_const_0 = Integer(0)
...
class SecureSignService:
    def __init__(self):
        ...
        self.a = EC.a
        self.b = EC.b
        self.privkey = random.randrange(_sage_const_1 , self.order - _sage_const_1 )
        self.pubkey = (self.privkey * self.G)
        self.key = os.urandom(_sage_const_16 )
        self.iv = os.urandom(_sage_const_16 )

    def split_privkey(self, privkey):
        shares = []
        coeffs = [privkey]
        for _ in range(_sage_const_3 ):
            coeffs.append(random.randrange(_sage_const_1 , self.order))

        P = PolynomialRing(GF(self.order), names=('x',)); (x,) = P._first_ngens(1)
        poly = sum(c*x**i for i, c in enumerate(coeffs))

        for x in range(_sage_const_1 , _sage_const_5 ):
            y = poly(x=x)
            shares.append((x, y))
        return shares
    ...
    def reconstruct_privkey(self, shares):
        ...
        return int(reconst_poly(_sage_const_0 ))

    def shares_encrypt(self, shares):
        return [self.aes_encrypt(long_to_bytes(int(s[_sage_const_1 ]))).hex() for s in shares]

    def shares_decrypt(self, shares):
        return [(x+_sage_const_1 , bytes_to_long(self.aes_decrypt(bytes.fromhex(y)))) for x, y in enumerate(shares)]
    ...
    def ecdsa_sign(self, message, privkey):
        z = int(hashlib.sha256(message).hexdigest(), _sage_const_16 )
        k = random.randrange(_sage_const_1 , self.order - _sage_const_1 )
    ...
    def ecdsa_verify(self, message, r, s, pubkey):
        r %= self.order
        s %= self.order
        if s == _sage_const_0  or r == _sage_const_0 :
            return False
        z = int(hashlib.sha256(message).hexdigest(), _sage_const_16 )
    ...
    def aes_decrypt(self, ciphertext):
        tag, ct = ciphertext[:_sage_const_16 ], ciphertext[_sage_const_16 :]
        cipher = AES.new(self.key, AES.MODE_GCM, nonce=self.iv)
        plaintext = cipher.decrypt_and_verify(ct, tag)
        return plaintext

    def get_flag(self):
        key = hashlib.sha256(long_to_bytes(self.privkey)).digest()[:_sage_const_16 ]
        cipher = AES.new(key, AES.MODE_ECB)
        encrypted_flag = cipher.encrypt(pad(FLAG.encode(), _sage_const_16 ))
    ...
    5) quit : Quit\n")
        ...
        try:
            ...
            if 'option' not in inp:
                ...
                msg = bytes.fromhex(inp['msg'])
                r = int(inp['r'], _sage_const_16 )
                s = int(inp['s'], _sage_const_16 )
                px = int(inp['px'], _sage_const_16 )
                py = int(inp['py'], _sage_const_16 )
